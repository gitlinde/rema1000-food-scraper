		...
		for(let i = 0; i < categories.length; i++) {
			...
			for(let id in rema100Data[foodCategory][splitUrl]) {
				...
				function periodBeforeSecondLast(number) {
					...
					let secondLastIndex = numberStr.length - 2;
	
					let result = numberStr.slice(0, secondLastIndex) + '.' + numberStr.slice(secondLastIndex);
				...
				async function findCSSElement(CSSPath) {
					try {
						...
						if (element) {
							...
							// let htmlContent = await page.evaluate(el => el.innerHTML, element);
